...
import com.sample.impl.deck.RegularDeck;
 ...
 */
public class RegularDeckTest {
    private RegularDeck deck;
    ...
    public void deckEquals52Cards() {
        Assert.assertEquals(deck.getCards().size(), RegularDeck.FULL_DECK_CARD_COUNT);
    }
    ...
    public void deckCardPosition() {
        Random random = new Random();
        //use 4 random numbers between 22 and 28 to pick a pick a card and verify that it matchs the card searched for
        for (int i = 0; i < 4; i++) {
            int randomPosition = 22 + random.nextInt(6);
            Card card = deck.getCard(randomPosition);
            int position = deck.search(card);
            Assert.assertEquals(position, randomPosition);
        }
    }
}
